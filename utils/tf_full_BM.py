...
class Workspace(object):
	def __init__(self, computecpp_root, workspace, eigen_branch, tf_branch, dlbench_branch, benchmarks_branch):
		...
		self.computecpp = self.fetch("http://computecpp.codeplay.com/downloads/computecpp-ce/latest/Ubuntu-16.04-64bit.tar.gz", workspace = self.tmp, dir="computecpp", file="Ubuntu-16.04-64bit.tar.gz")
		self.tensorflow = Repo("https://github.com/lukeiwanski/tensorflow.git", tf_branch, self.tmp)
		self.benchmarks = Repo("https://github.com/tensorflow/benchmarks.git", benchmarks_branch, self.tmp)
		self.dlbench    = Repo("https://github.com/tfboyd/dlbench.git", dlbench_branch, self.tmp)
	...
	def fetch(self, url, workspace, dir, file, delete = False):
		print("Fetching "+ url)
		path = workspace+"/"+dir
		cmd = "cd " + workspace

		# fetch tar.gz
		if not os.path.exists(workspace + "/" + file):
			cmd += " && wget " + url

		# make temp
		if not os.path.exists(path + "_tmp"):
			os.mkdir(path + "_tmp")
			cmd += " && tar xf " + file + " -C " + dir + "_tmp"
		...
		if delete:
			if os.path.exists(path):
				...
				print(bcolors.OKBLUE + "DONE!" + bcolors.ENDC)
				#full cmd
				full_cmd = "cd " + workspace + " && wget " + url + " && tar xf " + file + " -C " + dir + "_tmp"
	...
	def execute(self, cmd, log_modifier, cwd = "", my_env = []):
		print(bcolors.WARNING + cmd + bcolors.ENDC)
		p = subprocess.Popen(cmd, env=my_env, cwd=cwd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin=subprocess.PIPE)
	...
	def run_benchmarks(self):
		...
		# using gpu device is more flexible here - node that suppose to be accelerated on GPU will use SYCL device when aviable
		targets = ["cpu", "gpu"]
...
def main():
	...
	workspace = Workspace(computecpp_root = args.computecpp, workspace = args.workspace, eigen_branch = args.eigen, tf_branch = args.tf, benchmarks_branch = args.benchmarks, dlbench_branch = args.dlbench)
	workspace.setup()
